...
#   3. ~/.config/azur/claude-code-oauth-token.env         — existing convention on some machines
#   4. ~/.zshenv                                          — legacy fallback (grepped, not sourced)
for _tok in "${PERMA_TOKEN_ENV:-}" \
            "$HOME/.config/perma/claude-code-oauth-token.env" \
            "$HOME/.config/azur/claude-code-oauth-token.env"; do
